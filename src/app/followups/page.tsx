...
function FollowupsContent() {
  ...
  const executarAutomacao = async () => {
    ...
    const { data: response, error } = await fetchApi<{
      ...
      data: any;
      message: string;
    }>('/api/followups/automacao', { method: 'POST' });
  ...
  // Enriquecer leads com dados da planilha
  const enriquecerLeads = async () => {
    ...
    const { data: response, error } = await fetchApi<{
      ...
      message: string;
    }>('/api/enriquecer', { method: 'POST' });

    if (error) {
      setError(error);
    } else if (response?.success) {
      alert(`✅ ${response.message}\n\n📊 Detalhes:\n- Planilha: ${response.data.totalPlanilha} registros\n- Leads no CRM: ${response.data.totalLeads}\n- Atualizados: ${response.data.atualizados}\n- Não encontrados: ${response.data.naoEncontrados}`);
    }
  ...
  return (
    <div className="p-4 lg:p-6 min-h-screen bg-gray-50">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        >
          <Zap className={clsx('w-4 h-4', isRunningAutomation && 'animate-pulse')} />
          <span>{isRunningAutomation ? 'Executando...' : 'Rodar Automação'}</span>
        </button>

        {/* Botão de enriquecimento */}
        <button
          onClick={enriquecerLeads}
          disabled={isEnriquecendo}
          className="btn-secondary flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white border-purple-600"
        >
          <Database className={clsx('w-4 h-4', isEnriquecendo && 'animate-spin')} />
          <span>{isEnriquecendo ? 'Enriquecendo...' : 'Enriquecer Leads'}</span>
        </button>
        ...
        {followups.length === 0 ? (
          <div className="card p-8 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">
              Clique em &quot;Rodar Automação&quot; para criar follow-ups automáticos
            </p>
          </div>
        ) : (
