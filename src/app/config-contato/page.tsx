...
// ===========================================
// Tela: /config-contato  (CONFIG_CONTATO_V1 / CONTATADOS_COL_V1)
// Esquerda: configuração do contato automático da Tatiane.
// Direita: coluna com os leads que a Tati já contatou (nome, código, telefone).
// ===========================================
...
import {
  Send, Loader2, Save, CheckCircle, AlertCircle, Info, Minus, Plus, MessageCircle, RefreshCw,
} from 'lucide-react';
...
function ConfigContatoInner() {
  ...
  const carregarContatados = useCallback(async () => {
    ...
    const { data } = await fetchApi<{ success: boolean; hoje: number; data: Contatado[] }>('/api/tatiane-outbound-contatados?limite=50');
  ...
  const salvar = async () => {
    ...
    const { error } = await fetchApi('/api/tatiane-outbound-config', {
      method: 'PUT',
      body: JSON.stringify({ ativo: cfg.ativo, leads_por_dia: cfg.leads_por_dia, dias_apos_chegada: cfg.dias_apos_chegada }),
    });
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 items-start">
          ...
          {carregando ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div><div className="text-sm font-medium text-gray-900">Ativar contato automático</div><div className="text-xs text-gray-500 mt-0.5">Quando desligado, a Tati não inicia nenhum contato novo.</div></div>
                <button type="button" onClick={() => setCfg(c => ({ ...c, ativo: !c.ativo }))} className={`relative w-12 h-7 rounded-full transition-colors ${cfg.ativo ? 'bg-purple-600' : 'bg-gray-300'}`} aria-pressed={cfg.ativo}>
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div><div className="text-sm font-medium text-gray-900">Dias após a chegada do lead</div><div className="text-xs text-gray-500 mt-0.5">Ex.: 2 = contatar quem chegou há 2 dias (D-2).</div></div>
              ...
              <div className="px-5 py-4">
                <div className="bg-purple-50 border border-purple-100 rounded-xl px-4 py-3">
                  ...
                  <div className="text-sm text-gray-700 mt-1.5 leading-relaxed">
                    A Tati vai iniciar contato com leads que chegaram em <b>{formatarDia(cfg.dias_apos_chegada)}</b> — no máximo{' '}
                    <span className="inline-block bg-purple-600 text-white font-semibold rounded-full px-2 py-0.5 text-xs">{cfg.leads_por_dia} leads</span>. Quem já falou com a Tati é pulado e não gasta a cota.
                  </div>
                </div>
                <div className="flex items-start gap-2 text-xs text-gray-500 mt-3"><Info className="w-4 h-4 flex-none mt-0.5" /><span>Envios sempre na janela 8h–20h (Salvador). Cada lead é contatado uma única vez.</span></div>
              </div>
              <div className="flex items-center gap-3 px-5 pb-5">
                <button type="button" onClick={salvar} disabled={salvando} className="inline-flex items-center gap-2 bg-purple-600 text-white rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-purple-700 disabled:bg-gray-300">
                  {salvando ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}{salvando ? 'Salvando...' : 'Salvar configuração'}
                </button>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div><div className="text-sm font-semibold text-gray-900">Contatados pela Tati</div><div className="text-xs text-gray-500 mt-0.5">Leads que já receberam o primeiro contato</div></div>
              <div className="flex items-center gap-2">
            ...
            <div className="max-h-[480px] overflow-auto">
              {carregandoLista ? (
                ...
                contatados.map((c) => (
                  <div key={c.cod} className="flex items-center justify-between px-5 py-3 border-b border-gray-50">
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-gray-900 truncate">{c.nome || 'Sem nome'}</div>
                      <div className="text-xs text-gray-500 mt-0.5">{formatFone(c.celular)}{c.regiao ? ` • ${c.regiao}${c.estado ? ' ' + c.estado : ''}` : ''}</div>
                    </div>
                    <span className="text-[11px] font-semibold text-purple-700 bg-purple-50 rounded-md px-2 py-1 flex-none ml-2">{c.cod}</span>
                  </div>
                ))
              )}
            </div>
